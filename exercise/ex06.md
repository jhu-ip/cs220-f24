...
### Part 1
1.	Log into an undergraduate cluster computer (`ugradx.cs.jhu.edu`).
...
3.	Copy the template files to the current directory using: `cp ~/cs220-f24-public/exercises/ex06/* .`.
...
1.	Open the file for editing using `emacs compound.c` (or `vim compound.c`, if you use `vim`; `nano compound.c` if you want to use `nano`.)
...
3.	Modify the source files and test by:
	*	Compiling with `gcc -Wall -Wextra -std=c99 -pedantic compound.c -o compound -lm` or if you prefer to take advantage of the alias: `gccc compound.c -o compound -lm`.  If you forgot to setup the `gccc` alias, you can find instructions in [the previous exercise](ex05.md).
